	...
	<view class="centent">
		<scroll-view :style="{ height: pageHeight + 'px' }" lower-threshold="100" scroll-y="true" @scrolltolower="scrolltolower">
			...
			<view class="query-wrap">
				...
				<view class="query-down" :class="{ show: showModal }">
					<view class="query-scroll">
						<view v-for="(item, index) in checkoption" :key="index" class="bottom-inner query-list">
			...
			<view class="team-list-wrap">
				<view v-if="teamList.length === 0" class="empty"><image class="empty-image" src="../../static/images/State_empty.png" mode="aspectFit"></image></view>
				<view v-else v-for="(item, index) in teamList" :key="index" class="team-list bottom-inner">
...
export default {
	...
	methods: {
		getTeamList(pageNo) {
			const { searchStr, dqbm, expertise } = this;
			uni.request({
				...
				success: res => {
					const { count, pageNo, pageSize, pageCount, result } = res.data.result;
					console.log(result);
		...
		changeArea(item) {
			const { areaList, expertiseList } = this;
			console.log(areaList, expertiseList);
			if (item.value === 'area') {
				this.checkoption = areaList;
				this.checked = this.areaCheckIndex;
				this.checkName = 'area';
			} else if (item.value === 'expertise') {
				this.checkoption = expertiseList;
		...
		changeChecked(index) {
			if (this.checkName === 'area') {
				...
				this.dqbm = this.areaList[index].code;
			} else if (this.checkName === 'expertise') {
				this.expertiseCheckIndex = index;
				this.expertise = this.expertiseList[index].id;
			} else {
		...
		getAreaList() {
			uni.request({
				...
				success: res => {
					...
					const shaanxi = res.data.children.find(item => item.code === '610000');
					console.log(shaanxi);
					this.areaList = [{ name: '全部', code: '' }, ...shaanxi.children];
				}
			});
		},
		getExpertiseList() {
			uni.request({
				...
				success: res => {
					console.log(res);
					this.expertiseList = [{ name: '全部', id: '' }, ...res.data.children];
				}
			});
		}
	}
